...
if((int)phpversion() >= 7) {
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo "<center> id: " . $row["CustomerID"]. "- CompanyName: " . $row["CompanyName"]. "- ContactName: " . $row["ContactName"]. "- ContactTitle: " . $row["ContactTitle"]. "- Address: " . $row["Address"]. "- City: " . $row["City"]. "- Region: " . $row["Region"]. "- PosttalCode: " . $row["PosttalCode"]. "- Country: " . $row["Country"]. "- Phone: " . $row["Phone"]."- Fax: " . $row["Fax"]. "<br/>";
		}
	} else {
		echo "no results";
	}
	...
	while ($object = mysql_fetch_object($result)) {
		echo $object->CustomerID . "  " . $object->CompanyName . "<br/>";
	}
