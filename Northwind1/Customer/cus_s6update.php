...
$sql.="update cusducts set ";
$sql.="CustomerID =". $_GET['cid'] .", ";
$sql.="CompanyName ='". $_GET['coname'] ."', ";
$sql.="ContactName ='". $_GET['conname'] ."' ";
$sql.="ContactTitle ='". $_GET['ctt'] ."' ";
$sql.="Address ='". $_GET['add'] ."' ";
$sql.="City ='". $_GET['ct'] ."' ";
$sql.="Region ='". $_GET['rg'] ."' ";
$sql.="PosttalCode ='". $_GET['po'] ."' ";
$sql.="Country ='". $_GET['co'] ."' ";
$sql.="Phone ='". $_GET['ph'] ."' ";
$sql.="Fax ='". $_GET['fa'] ."' ";
$sql.="where CustomerID =". $_GET['cid'] ." ";
if((int)phpversion() >= 7) {
	if($connect->query($sql) === FALSE) 
		echo "$sql : failed";
	else 
		echo "<meta http-equiv='refresh' content='0; url=cus_s6update.php'/>";	
	$connect->close();
} else {	
...
echo "<center><a href=cus_s3select.php>back</a>";
?>
</header>
