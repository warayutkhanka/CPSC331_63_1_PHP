...
if((int)phpversion() >= 7) {
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo "<center> id: " . $row["SupplierID"]. "- CompanyName: " . $row["CompanyName"]. "- ContactName: " . $row["ContactName"]."- ContactTitle: " . $row["ContactTitle"]."- Address: " . $row["Address"]."- City: " . $row["City"]."- Region: " . $row["Region"]."- PostalCode: " . $row["PostalCode"]."- Country: " . $row["Country"]."- Phone: " . $row["Phone"]."- Fax: " . $row["Fax"]."- HomePage: " . $row["HomePage"]. "<br/>";
		}
	} else {
		echo "no results";
	}
	...
	echo "Display all recsups : <br/>";
	while ($object = mysql_fetch_object($result)) {
		echo $object->SupplierID . "  " . $object->CompanyName . "<br/>";
	}
	echo "Total " . mysql_num_rows($result) ." recsups";
	mysql_close($connect);
}
